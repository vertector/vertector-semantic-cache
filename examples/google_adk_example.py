...
async def cache_sharing_example():
    """Demonstrate cache sharing with semantic similarity matching."""
    print("\n" + "="*70)
    print("Cache Sharing Example")
    print("="*70 + "\n")
    ...
    async with cache_manager:
        ...
        # Query 1: First query (cache miss, calls LLM)
        print("1. First query: 'What is the capital of Ghana?'")
        response1 = await cached_agent.query("What is the capital of Ghana?")
        print(f"   Response: {response1[:100]}...\n")
        
        # Query 2: Same exact question (cache HIT - instant response!)
        print("2. Same question again (should use cache):")
        response2 = await cached_agent.query("What is the capital of Ghana?")
        print(f"   Response: {response2[:100]}...\n")
        
        # Query 3: Semantically similar question (cache HIT via semantic matching!)
        print("3. Similar question: 'Tell me Ghana's capital city'")
        response3 = await cached_agent.query("Tell me Ghana's capital city")
        print(f"   Response: {response3[:100]}...\n")
        
        print("✅ Cache sharing example complete!\n")
        await cached_agent.close()
